    ...
    <main className="grid min-h-screen grid-cols-1 bg-[#f8f3ed] text-[#3b2a29] min-[821px]:grid-cols-[minmax(400px,46%)_1fr]">
      <section className="relative flex min-h-[440px] flex-col overflow-hidden bg-[#b9827e] px-[6vw] py-7 text-[#fffaf5] min-[821px]:min-h-0 min-[821px]:px-[6vw] min-[821px]:py-12" aria-label="Veloura skincare introduction">
        ...
        <div className="relative z-10 my-[70px] max-w-[560px] min-[821px]:my-auto">
          <p className="mb-3.5 text-xs font-bold uppercase tracking-[.16em] text-[#f5d9ce]">A softer approach to beauty</p>
          <h1 className="mb-5 max-w-[560px] font-['Playfair_Display'] text-[2.7rem] font-medium leading-[1.06] text-[#fffaf5] min-[481px]:text-[3.1rem] min-[821px]:text-[clamp(2.6rem,3.7vw,4.1rem)]">Begin your glow ritual.</h1>
          <p className="max-w-[480px] text-[15px] leading-[1.75] text-[#f8e8e1]">Meet carefully chosen essentials for skin that feels nourished, balanced, and beautifully cared for.</p>
        </div>
        ...
        <blockquote className="relative z-10 m-0 max-w-[330px] border-l border-[#f1d2c8] pl-4 text-[13px] leading-[1.65] text-[#f8e8e1] max-[820px]:hidden">
          <p className="mb-3 italic">"Your everyday care, made beautiful."</p>
      ...
      <section className="grid place-items-center px-[8vw] py-[54px] min-[821px]:px-[7vw] min-[821px]:py-[52px]">
        <div className="w-full max-w-[480px]">
          <div className="mb-8">
            <p className="mb-3.5 text-xs font-bold uppercase tracking-[.16em] text-[#a86f6b]">A ritual made for you</p>
            <h2 className="mb-2 font-['Playfair_Display'] text-[clamp(2rem,3vw,2.55rem)] font-medium leading-[1.15] text-[#3b2a29]">Welcome to Veloura</h2>
            <p className="text-sm text-[#806967]">Create your account and discover a gentler way to care for your skin.</p>
          </div>

          <form className="grid gap-[9px]" onSubmit={handleSubmit} noValidate>
            <label className="text-[13px] font-semibold text-[#5b4240]" htmlFor="register-fullName">Full name</label>
            <input className="mb-[9px] box-border h-[50px] w-full rounded-[13px] border border-[#e3d2c8] bg-[#fffdf9] px-4 text-sm text-[#3b2a29] outline-none placeholder:text-[#b5a09a] focus:border-[#b9827e] focus:ring-[3px] focus:ring-[rgba(185,130,126,.15)]" id="register-fullName" name="fullName" type="text" placeholder="Your name" autoComplete="name" required />

            <label className="text-[13px] font-semibold text-[#5b4240]" htmlFor="register-email">Email address</label>
            <input className="mb-[9px] box-border h-[50px] w-full rounded-[13px] border border-[#e3d2c8] bg-[#fffdf9] px-4 text-sm text-[#3b2a29] outline-none placeholder:text-[#b5a09a] focus:border-[#b9827e] focus:ring-[3px] focus:ring-[rgba(185,130,126,.15)]" id="register-email" name="email" type="email" placeholder="you@example.com" autoComplete="email" required />

            <div className="grid grid-cols-1 gap-0 min-[481px]:grid-cols-2 min-[481px]:gap-3.5">
              <div>
                <label className="text-[13px] font-semibold text-[#5b4240]" htmlFor="register-password">Password</label>
                <div className="relative">
                  <input className="mb-[9px] box-border h-[50px] w-full rounded-[13px] border border-[#e3d2c8] bg-[#fffdf9] px-4 pr-[54px] text-sm text-[#3b2a29] outline-none placeholder:text-[#b5a09a] focus:border-[#b9827e] focus:ring-[3px] focus:ring-[rgba(185,130,126,.15)]" id="register-password" name="password" type={showPassword ? 'text' : 'password'} placeholder="At least 8 characters" autoComplete="new-password" required />
              ...
              <div>
                <label className="text-[13px] font-semibold text-[#5b4240]" htmlFor="register-confirmPassword">Confirm password</label>
                <input className="mb-[9px] box-border h-[50px] w-full rounded-[13px] border border-[#e3d2c8] bg-[#fffdf9] px-4 text-sm text-[#3b2a29] outline-none placeholder:text-[#b5a09a] focus:border-[#b9827e] focus:ring-[3px] focus:ring-[rgba(185,130,126,.15)]" id="register-confirmPassword" name="confirmPassword" type={showPassword ? 'text' : 'password'} placeholder="Repeat password" autoComplete="new-password" required />
              </div>
            </div>

            {(error || apiError) && <p className="my-0 text-xs text-[#ae4949]" role="alert">{error || apiError}</p>}
            {submitted && <p className="my-0 text-xs text-[#56806b]" role="status">Account created successfully. You can log in now.</p>}
            <button className="mt-1 h-[50px] rounded-[13px] border-0 bg-[#6d4946] text-[13px] font-bold text-[#fffaf5] shadow-[0_8px_18px_rgba(109,73,70,.18)] transition hover:-translate-y-px hover:bg-[#583a38] disabled:cursor-not-allowed disabled:opacity-60" type="submit" disabled={isLoading}>{isLoading ? 'Creating account...' : 'Create account'} <span className="ml-2 text-[17px]" aria-hidden="true">→</span></button>
          </form>

          <p className="mt-[23px] text-center text-[13px] text-[#806967]">Already part of Veloura? <Link className="font-bold text-[#a86f6b]" to="/login">Log in</Link></p>
        </div>
      </section>
    </main>
  )
